package uz.aquabill.app.v1.product.mapper
...
import uz.aquabill.app.v1.product.dto.ProductDto
...
class ProductMapper {
    ...
    fun toResponse(product: Product): ProductResponse {
        ...
        return ProductResponse(
            ...
            active = product.active,
            createdAt = product.createdAt?.let { LocalDateTime.ofInstant(it, ZoneId.systemDefault()) },
            updatedAt = product.updatedAt?.let { LocalDateTime.ofInstant(it, ZoneId.systemDefault()) }
        )
    }
    ...
    fun toUnitDto(units: Units): UnitDto {
        ...
        return UnitDto(
            ...
            name = units.name ?: "",
            active = units.active,
            createdAt = units.createdAt ?: Instant.now(),
            updatedAt = units.updatedAt ?: Instant.now()
        )
    }
    ...
    fun toUnitEntity(unitDto: UnitDto): Units {
        return Units().apply {
            ...
            createdAt = unitDto.createdAt ?: Instant.now()
            updatedAt = unitDto.updatedAt ?: Instant.now()
        }
    }
